...
def upload_file():
    ...
    if file and file.filename.endswith('.csv'):
        ...
        response = jsonify({"success": True, "predicted": pred_list, "message": "Valid file type."})
...
def get_pred_values():
    global predicted_values
    with lock:
        response = jsonify({"success": True, "predicted": predicted_values, "message": "Valid file type."})
    response.headers.add('Access-Control-Allow-Origin', '*')  # Allow requests from any origin
    return response
